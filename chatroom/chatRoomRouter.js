  ...
  Chatroom.create({users: req.user.id, asker: req.user.id})
    .then((result) => {
      ...
      Chatroom.findOneAndUpdate(id, {url})
        .then((result) => {
          res.location(`${req.originalUrl}/${result.id}`).status(201).json({url, id, questions});
          return result;
        });      
...
router.put('/join-room', jsonParser, jwtAuth, missingField, getQuestions, (req, res, next) => {
  ...
  Chatroom.findOne({url})
    .then(result => {
      if(!result.users.toString().includes(req.user.id)){
        console.log('it doesnt include it!');
...
router.put('/leave-room', jwtAuth, missingField, (req, res, next) => {
  const {url} = req.headers;
  Chatroom.findOneAndUpdate({url}, {$pull: {users: req.user.id}, active: false}, {new: true})
    .then((response) => {
      // if(response.users.length === 0){
      //   Chatroom.findOneAndUpdate({url}, {active: false}, {new: true})
